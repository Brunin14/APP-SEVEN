...
import { useRouter } from "expo-router"; // 2. Trocado Link por useRouter
...
export default function ComprasScreen() {
  const [lists, setLists] = useState<ListsState>(INITIAL_LISTS_STATE);
  ...
  const router = useRouter(); // 3. Hook do router
  const insets = useSafeAreaInsets(); // Hook para a área segura
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (raw) {
          const loadedLists = JSON.parse(raw);
          setLists({ ...INITIAL_LISTS_STATE, ...loadedLists });
        }
      } catch (error) {
    ...
    function RenderItem({ item }: ListRenderItemInfo<Item>) {
      return (
        <View style={styles.row}>
          ...
          <TouchableOpacity style={styles.itemTextWrap} onLongPress={() => startEdit(loc, item)}>
          ...
          <TouchableOpacity onPress={() => startEdit(loc, item)} style={styles.iconBtn}>
            <Ionicons name="create-outline" size={20} color="#374151" />
          </TouchableOpacity>
          <TouchableOpacity onPress={() => removeItem(loc, item.id)} style={styles.iconBtn}>
    ...
    <View style={styles.screen}>
      ...
      <KeyboardAvoidingView behavior={Platform.select({ ios: "padding", android: undefined })} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.container}>
          {/* 5. TÍTULO DA PÁGINA MOVIDO PARA CÁ */}
          <View style={styles.hero}>
            <Text style={styles.title}>Lista de Compras</Text>
            <Text style={styles.subtitle}>Adicione e gerencie os itens de cada local.</Text>
          </View>
...
function EditModal({ visible, initialName, initialQty, onCancel, onSave }: {
  visible: boolean;
  initialName: string;
  initialQty?: string;
  onCancel: () => void;
...
const styles = StyleSheet.create({
  ...
  // 6. ESTILOS ATUALIZADOS E ADICIONADOS
  header: {
    backgroundColor: "#fff",
    paddingBottom: 10,
