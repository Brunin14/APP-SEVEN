...
import {
  ...
  Platform, // <-- CORRIGIDO: Importação do Platform adicionada
} from "react-native";
import { FontAwesome, Ionicons } from "@expo/vector-icons";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { useRouter } from "expo-router";
// <-- NOVO: Importando AsyncStorage para ser consistente com o AuthContext
...
// Formulário extraído para seu próprio componente (sem alterações)
...
// Componente para a lista vazia (sem alterações)
...
export default function ComunicadosPage() {
  ...
  const enviarComunicado = useCallback(async () => {
    ...
    setSubmitting(true);
    console.log("[DEBUG] 1. Iniciando envio do comunicado...");

    try {
      // <-- CORRIGIDO: Buscando o token do AsyncStorage com a chave correta ("auth_token")
      const token = await AsyncStorage.getItem('auth_token');
      
      console.log("[DEBUG] 2. Token encontrado:", token ? `Sim (termina com ...${token.slice(-4)})` : "Não");

      if (!token) {
        Alert.alert("Erro de Autenticação", "Sessão inválida. Por favor, faça login novamente.");
        router.replace('/login');
        setSubmitting(false); // Garante que o estado de submitting seja resetado
        return;
      }
      
      console.log("[DEBUG] 3. Realizando chamada fetch para a API...");
      ...
      console.log("[DEBUG] 4. Resposta da API recebida.", { status: res.status, ok: res.ok });

      if (!res.ok) {
        if (res.status === 401 || res.status === 403) {
           throw new Error("Sua sessão expirou ou você não tem permissão para esta ação.");
        }
        const errorData = await res.json().catch(() => ({ error: "A resposta do servidor não é um JSON válido." }));
        throw new Error(errorData?.error || "Falha ao salvar o comunicado.");
      }

      console.log("[DEBUG] 5. Analisando JSON da resposta...");
      const novo = await res.json();
      console.log("[DEBUG] 6. JSON analisado com sucesso:", novo);
      ...
      console.error("[DEBUG] 7. ERRO CAPTURADO no bloco catch:", err);
      Alert.alert("Erro", err.message || "Não foi possível enviar o comunicado.");
    } finally {
      console.log("[DEBUG] 8. Bloco finally executado. Resetando 'submitting'.");
      setSubmitting(false);
    }
  }, [titulo, corpo, submitting, router]);
